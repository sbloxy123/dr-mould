...
**Analytics and consent:** `components/GoogleAnalytics.jsx` (measurement ID `G-KSTFZWW3Y6`) uses gtag consent mode. Its inline script reads the stored choice (`cookie_consent` in localStorage) and sets the consent default before `config`, so returning visitors who accepted are counted from their first page view; the `ad_*` types are always denied. `config` sends the first page view and `components/PageviewTracker.tsx` sends one per client-side path change. **GA's "Page changes based on browser history events" (Enhanced measurement → Page views) must stay off**, or every navigation is counted twice. gtag.js only loads when `NODE_ENV` is production and `VERCEL_ENV` isn't `preview` (`app/layout.tsx`), so Previews and `next dev` fill the dataLayer but send nothing. `CookieBanner` shows until a choice is made, only writes storage and updates consent on click, and clears the `_ga` cookies on Decline. The footer's `CookieSettingsButton` reopens it via the `cookie-consent:open` event. Shared helpers and constants are in `lib/analytics.ts`; push to the dataLayer with its `gtag()`, not `window.gtag`.

**SEO:** each page exports `metadata` built with `pageMetadata()` from `utils/metadata.ts` (title, description, its own canonical URL and Open Graph tags). `metadataBase` (`https://www.dr-mould.co.uk`) and the JSON-LD are in `app/layout.tsx`. `app/robots.ts` serves `/robots.txt`, which points crawlers at `app/sitemap.ts`; both build their URLs from `site.url`. `public/google3b718850404d2b8d.html` is the Google Search Console ownership file: keep it, or the site loses its verification. The social sharing image is `public/og-image.png` (1200 x 630), set once as `ogImage` in `utils/metadata.ts` and used for Open Graph, Twitter (via `summary_large_image`, which Next fills from Open Graph) and the JSON-LD. Its source is `redesign/og-image.html`, which has the command to regenerate it; the logo is `public/logo.svg` (and `logo.png` for JSON-LD), and `app/apple-icon.png` is the touch icon.
