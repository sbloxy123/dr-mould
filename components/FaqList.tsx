...
export default function FaqList({ items, className }: FaqListProps) {
  return (
    <div data-slot="faq-list" className={cn("flex flex-col border-t border-sand-300", className)}>
      {items.map((item, index) => (
        ...
        >
          <summary data-slot="faq-question" className="flex cursor-pointer list-none items-start justify-between gap-3 rounded-md text-[17px] font-semibold leading-snug text-forest-900 lg:text-xl [&::-webkit-details-marker]:hidden">
            {item.question}
            <span
              aria-hidden
          ...
          <div data-slot="faq-answer" className="flex max-w-[720px] flex-col gap-3 pt-3 text-base leading-[1.6] text-ink-700 lg:pt-3.5 lg:text-[17px] lg:leading-[1.65]">
