    ...
    <article id="causes" className="flex flex-col gap-4 lg:gap-[22px]">
      ...
      <div className="grid grid-cols-2 gap-2.5 lg:gap-4 lg:pt-1.5">
        {causes.items.map((item) => (
          <div
            key={item.title}
            className="flex flex-col gap-1 rounded-[14px] border border-sand-200 bg-paper px-3.5 py-4 min-[360px]:px-[18px] lg:gap-2 lg:rounded-2xl lg:px-6 lg:py-[22px]"
          >
            <h3 className="hyphens-auto text-base font-semibold text-forest-900 min-[360px]:text-[17px] lg:text-lg">
              {item.title}
            </h3>
